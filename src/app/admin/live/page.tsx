...
export default async function AdminLivePage() {
  ...
  const instructors = await prisma.user.findMany({
    ...
    select: { id: true, name: true, email: true, maxLiveStreams: true },
    orderBy: { name: 'asc' }
  });
  ...
  return (
    <div style={{ padding: '2rem 5%', minHeight: '100vh', background: '#050505', color: '#fff' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: 'bold' }}>إدارة البث المباشر</h1>
      ...
      <LiveClient initialStreams={streams} initialInstructors={instructors} />
    </div>
  );
}
